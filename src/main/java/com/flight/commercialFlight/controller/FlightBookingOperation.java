...
public class FlightBookingOperation {
    ...
    public ResponseEntity<Mono<BaseResponse>> updateflightinfo(@RequestHeader HttpHeaders headers, @RequestBody FlightDetails flightDetails, @PathVariable("flight_id") String flightId) {
    ...
    public ResponseEntity<Mono<BaseResponse>> getallflights(@RequestHeader HttpHeaders headers) {
        return ResponseEntity.ok(flightManagementService.fetchAllFlightInformation());
    }

    @GetMapping(FETCH_FLIGHT_DETAIL)
    public ResponseEntity<Mono<BaseResponse>> getflightdetail(@RequestHeader HttpHeaders headers, @PathVariable("flight_id") String flightId) {
        return ResponseEntity.ok(flightManagementService.fetchFlightbyId(flightId));
    }

    @GetMapping(FETCH_PASSENGER_ALL)
    public ResponseEntity<Mono<BaseResponse>> getallpassengerinfo(@RequestHeader HttpHeaders headers) {
        return ResponseEntity.ok(passengerService.getAllPassengers());
    }

    @PostMapping(REGISTER_PASSENGER)
    public ResponseEntity<Mono<BaseResponse>> registerpassenger(@RequestHeader HttpHeaders headers, @RequestBody PassengerDetails passengerDetails) {
        return ResponseEntity.ok(passengerService.registerPassenger(passengerDetails));
    }

    @PatchMapping(UPDATE_PASSENGER)
    public ResponseEntity<Mono<BaseResponse>> updatepassenger(@RequestHeader HttpHeaders headers, @RequestBody PassengerDetails passengerDetails) {
        return ResponseEntity.ok(passengerService.updatePassengerDetails(passengerDetails));
    }

    @GetMapping(FETCH_PASENG_ID)
    public ResponseEntity<Mono<BaseResponse>> getpassengerdetails(@RequestHeader HttpHeaders headers, @PathVariable("passenger_id") String passengerId) {
        return ResponseEntity.ok(passengerService.getPassengerInfo(passengerId));
    }

    @PostMapping(CREATE_BOOKING)
    public ResponseEntity<Mono<BaseResponse>> createbooking(@RequestHeader HttpHeaders headers, @PathVariable("flight_id") String flightId, @PathVariable("passenger_id") String passengerId) throws InterruptedException {
        return ResponseEntity.ok(bookingService.makeFlightBooking(flightId, passengerId));
    }

    @GetMapping(ALL_BOOKING)
    public ResponseEntity<Mono<BaseResponse>> getallbookingdetails(@RequestHeader HttpHeaders headers) {
        return ResponseEntity.ok(bookingService.getAlltheBookings());
    }

    @GetMapping(BOOKING_BY_FLIGHT_NUM)
    public ResponseEntity<Mono<BaseResponse>> getbookingbyflight(@RequestHeader HttpHeaders headers, @PathVariable("flight_id") String flightId) {
        return ResponseEntity.ok(bookingService.viewBookingByFlight(flightId));
    }

    // Hard delete
    @DeleteMapping(CANCLE_BOOKING)
    public ResponseEntity<Mono<BaseResponse>> canclebooking(@RequestHeader HttpHeaders headers, @PathVariable("booking_id") String bookingId) {
        return ResponseEntity.ok(bookingService.cancelFlightBooking(bookingId));
    }

    @GetMapping(BOOKING_BY_PASSENGER_NUM)
    public ResponseEntity<Mono<BaseResponse>> getbookingbypassenger(@RequestHeader HttpHeaders headers, @PathVariable("passenger_id") String passengerId) {
        return ResponseEntity.ok(bookingService.viewBookingByPassenger(passengerId));
    }

    @GetMapping(SEARCH_FLIGHT)
    public ResponseEntity<Mono<BaseResponse>> getflightbydeptureanddest(@RequestHeader HttpHeaders headers, @PathVariable("departure") String dept, @PathVariable("destination")
    String dest) {
